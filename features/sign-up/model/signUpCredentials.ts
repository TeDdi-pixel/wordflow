...
export const signUpCredentials = async (
  ...
  if (formData.email === "" || formData.password === "")
    return {
      ...
      error: AUTH_ERROR_MESSAGES.MISSING_FIELDS,
    };
  ...
  if (!EMAIL_REGEX.test(formData?.email))
    return {
      ...userBackUpFields(formData),
      error: AUTH_ERROR_MESSAGES.INVALID_EMAIL,
    };
  ...
  if (
    ...
    return {
      ...userBackUpFields(formData),
      error: AUTH_ERROR_MESSAGES.CREDENTIALS_DO_NOT_MATCH,
    };

  if (formData.username === "admin")
    return {
      ...userBackUpFields(formData),
      error: AUTH_ERROR_MESSAGES.RESERVED_USERNAME,
    };

  if (formData.email === "admin@gmail.com")
    return {
      ...userBackUpFields(formData),
      error: AUTH_ERROR_MESSAGES.RESERVED_EMAIL,
    };
  ...
  try {
    ...
    if (existUserDoc) {
      return {
        ...userBackUpFields(formData),
        error: AUTH_ERROR_MESSAGES.EMAIL_IS_ALREADY_IN_USE,
      };
    }
    ...
    if (error instanceof AuthError) {
      switch (error.type) {
        ...
        default:
          return {
            ...userBackUpFields(formData),
            error: AUTH_ERROR_MESSAGES.SIGN_UP_ERROR,
          };
      }
    }
    throw error;
